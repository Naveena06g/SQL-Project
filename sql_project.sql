...
(20, '2023-08-20', 20, 1199.98),(21, '2023-08-21', 21, 875.00),
...
select customer.customer_name,count(orders.order_id) as order_count
from customer
...
where orders.order_date>='2023-10-09'- interval 30 day
order by
order_date desc;
...
ORDER BY 
    customer.customer_name, products.product_name;
...
SELECT 
     ...
     count(orders.order_id) as no_of_orders_placed
     from orders
GROUP BY
     month 
ORDER BY
...
GROUP BY 
    products.product_name
HAVING 
    COUNT(orderdetails.order_detail_id)<10
ORDER BY 
...
SELECT 
    ...
    orders.order_id,
    orders.order_date,
    count(distinct orderdetails.product_id) as product_count
FROM 
    orders
JOIN 
    customer on orders.customer_id=customer.customer_id
...
SELECT
   customer.customer_city,
   products.product_name,
   SUM(orders.order_id) as total_quantity
FROM 
   orders
JOIN
   customer on orders.customer_id=customer.customer_id   
JOIN
   products
GROUP BY
   customer.customer_city,products.product_name
ORDER BY
   total_quantity desc;
